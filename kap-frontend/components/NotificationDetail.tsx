...
const NotificationDetail: React.FC<Props> = ({ recentNotifications }) => {
   const { id } = useParams<{ id: string }>();
   const navigate = useNavigate();

   const [notification, setNotification] = useState<Notification | undefined>(
      recentNotifications.find(n => n.id === id)
   );
   ...
   useEffect(() => {
      ...
      const fetchSummary = async () => {
         try {
            const response = await fetch(`${API_BASE_URL}/prices/summary`);
            if (response.ok) {
      ...
      // Only fetch if we haven't already (or keep it simple and just fetch)
      fetchSummary();

      if (notification || !id) return;

      // Check recentNotifications again - maybe they loaded late
      const found = recentNotifications.find(n => n.id === id);
      if (found) {
      ...
      fetchNotification();
   }, [id, recentNotifications, notification]);


   if (loading) {
      return (
         <div className="min-h-screen bg-market-bg flex items-center justify-center">
            <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-market-accent"></div>
         </div>
      );
   }
      ...
      return (
         <div className="min-h-screen bg-market-bg flex items-center justify-center">
            <div className="text-center">
               <p className="text-market-text mb-4">Haber bulunamadı.</p>
               <button onClick={() => navigate('/')} className="text-market-accent underline">Ana Sayfaya Dön</button>
            </div>
         </div>
      )
   }
            ...
            <aside className="hidden lg:block space-y-8">
               <div className="sticky top-24">
                  ...
                  <div className="mt-6 bg-market-card border border-market-border rounded-lg shadow-sm p-5">
                     <h3 className="font-bold text-market-text text-sm mb-3 text-center">BIST Piyasa Özet</h3>
                     {marketSummary ? (
                        <div className="grid grid-cols-2 gap-2 text-center">
                           <div className="p-2 bg-market-green/10 rounded">
                              <div className="text-xs text-market-muted">Yükselen</div>
                              <div className="text-market-green font-bold text-lg">{marketSummary.rising}</div>
                           </div>
                           <div className="p-2 bg-market-red/10 rounded">
                              <div className="text-xs text-market-muted">Düşen</div>
                              <div className="text-market-red font-bold text-lg">{marketSummary.falling}</div>
                           </div>
                        </div>
                     ) : (
                        <div className="flex justify-center py-4">
                           <div className="animate-spin rounded-full h-6 w-6 border-t-2 border-b-2 border-market-muted"></div>
                        </div>
                     )}
                  </div>
               </div>
